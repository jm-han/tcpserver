...
static void setinput(struct in *_in, unsigned char *p)
{
    _in->p = p;
    _in->left = 0;
    _in->bits = 0;
...
static int fillbits(struct in *_in, int le, unsigned int bi)
{
    int b, m;

    if (_in->marker) {
        if (le <= 16)
            _in->bits = bi << 16, le += 16;
        return le;
    }
    while (le <= 24) {
        b = *_in->p++;
        if (b == 0xff && (m = *_in->p++) != 0) {
            if (m == M_EOF) {
                if (_in->func && (m = _in->func(_in->data)) == 0)
                    continue;
            }
            _in->marker = m;
            if (le <= 16)
    ...
    _in->bits = bi;		/* tmp... 2 return values needed */
    return le;
}

static int dec_readmarker(struct in *_in)
{
    int m;

    _in->left = fillbits(_in, _in->left, _in->bits);
    if ((m = _in->marker) == 0)
        return 0;
    _in->left = 0;
    _in->marker = 0;
...
static int dec_rec2(struct in *_in, struct dec_hufftbl *hu, int *runp, int c, int i)
{
    LEBI_DCL;
    ...
    if (i) {
        ...
        *runp = i >> 8 & 15;
        ...
        for (i = DECBITS;
                (c = ((c << 1) | GETBITS(_in, 1))) >= (hu->maxcode[i]); i++);
        if (i >= 16) {
            _in->marker = M_BADHUFF;
            return 0;
        }
...
static void decode_mcus(struct in *_in, int *dct, int n, struct scan *sc, int *maxp)
{
    ...
    while (n-- > 0) {
        hu = sc->hudc.dhuff;
        *dct++ = (sc->dc += DEC_REC(_in, hu, r, t));

        hu = sc->huac.dhuff;
        i = 63;
        while (i > 0) {
            t = DEC_REC(_in, hu, r, t);
            if (t == 0 && r == 0) {
                dct += i;
                break;
            }
