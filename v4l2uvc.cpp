...
unsigned long uvcGrabMJPEG(struct vdIn *vd, unsigned char* pData)
{
    int ret;

    qDebug() << "uvcGrabMJPEG 0";

    if (!vd->isstreaming)
        if (video_enable(vd))
            return 0;
    qDebug() << "uvcGrabMJPEG 1";
    memset(&vd->buf, 0, sizeof(struct v4l2_buffer));
    vd->buf.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
    vd->buf.memory = V4L2_MEMORY_MMAP;
    qDebug() << "uvcGrabMJPEG 2";
    ret = ioctl(vd->fd, VIDIOC_DQBUF, &vd->buf);
    qDebug() << "uvcGrabMJPEG 3";
    if (ret < 0) {
        printf("Unable to dequeue buffer (%d).\n", errno);
        return 0;
    }

    memcpy(pData, vd->mem[vd->buf.index], vd->buf.bytesused);
    ret = ioctl(vd->fd, VIDIOC_QBUF, &vd->buf);
    if (ret < 0) {
        qDebug() << "Unable to requeue buffer";
        return -1;
    }
